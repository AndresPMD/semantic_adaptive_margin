...
def main():
    ...
    parser.add_argument('--orig_img_path', default='./data/',
                        help='path to get the original image data')
    parser.add_argument('--orig_data_name', default='coco',
                        help='{coco,f30k}')
    ...
    parser.add_argument('--log_step', default=20, type=int,  help='Number of steps to print and record the log.')
    parser.add_argument('--val_step', default=1500, type=int,  help='Number of steps to run validation.')
    parser.add_argument('--logger_name', default='./runs/coco/CVSE_COCO/log',
                        help='Path to save Tensorboard log.')
    parser.add_argument('--model_name', default='./runs/coco/CVSE_COCO/',
                        help='Path to save the model.')
    parser.add_argument('--resume', default='./runs/coco/CVSE_COCO/model_best.pth.tar', type=str, metavar='PATH',
    ...
    parser.add_argument('--bi_gru', action='store_false',  help='Whether to use bidirectional GRU.')
    ...
    parser.add_argument('--attribute_path',
                        default='data/coco_annotations/Concept_annotations/',
                        help='path to get attribute annotation label json file')
    parser.add_argument('--inp_name',
                        default = 'data/coco_annotations/Concept_annotations/coco_concepts_glove_word2vec.pkl',
                        help = 'load the input glove word embedding file')
    parser.add_argument('--adj_file', default = 'data/coco_annotations/Concept_annotations/coco_adj_concepts.pkl',
    ...
    parser.add_argument('--wemb_type', default='random_init', choices=('glove', 'fasttext', 'random_init'), type=str,
                        help='Initial Word embedding for textual branch (glove|fasttext|random_init)')
...
def validate(opt, val_loader, model, M):
    ...
    if opt.use_metric:
        ...
        currscore = sum(scores['t2i']['softer'][i] for i in [1, 5, 10]) \
                    + sum(scores['i2t']['softer'][i] for i in [1, 5, 10])
