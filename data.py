...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, cap_json_path, data_path, attribute_path, data_split, vocab, opt):
        ...
        if data_split != 'train':
            ...
            image_len = self.images.shape[0]
        else:
            image_len = len(os.listdir(loc+'train'))
        ...
        self.data_percentage = float(self.data_percentage) / 100.0
    ...
    def __getitem__(self, index):
        ...
        if self.data_split != 'train':
            ...
            image = torch.Tensor(np.load(self.loc+'train/%s.npy' % img_id))
        ...
        # Additional Metric
        captions_gts = self.captions[img_id * 5:img_id * 5 + 5]

        return image, target, attribute_label, attri_input_emb, index, img_id, captions_gts, self.captions[index]
    ...
    def __len__(self):
        if self.data_split == 'train':
            return int (self.length * self.data_percentage)
        else:
            return self.length
...
def collate_fn(data):
    ...
    images, captions, attribute_label, attribute_input_emb, ids, img_ids, caption_gts, caption_batch = zip(*data)
    ...
    return images, targets, attribute_labels, attribute_input_embs, lengths, ids, caption_gts, caption_batch
...
def get_paths(path, attribute_path, name='coco', transfer_test=False, use_restval=True):
    ...
    if 'coco_precomp' == name:
        ...
        # load file path of train/val_annt.json of concept annotations'''
        roots_anno = {}
        if use_restval:
            roots_anno = {
        ...
        if transfer_test == False:
            ...
            # PROPER EVAL
            # roots_anno = os.path.join(attribute_path, 'trainval_concept_label.json')
